...
3 pages:
  Page 1 — Project summary (professor / technical)
  Page 2 — WHO early warning forecast (COVID dropdown + RSV Australia)
  Page 3 — Public situation (plain English, traffic light)
"""
...
# Fallback: look in the repo Data/ folder for large files not copied to dashboard/data
REPO_DATA = Path(__file__).parent.parent / "Data"

def find_file(filename):
    """Find a file in dashboard/data first, then repo Data/ folder."""
...
def trim_trailing_zeros(df, case_col="Cases"):
    """Remove trailing rows where cases are 0 — countries that stopped reporting show as flatline."""
...
if "Summary" in page:
    st.markdown('''<div class="who-bar"><h1>\U0001f9a0 Inferra \u2014 Project Summary</h1>
        <p>Respiratory disease surveillance · Australia · Updated fortnightly</p>
...
- Generalises across geographies and diseases""")
    ...
    for col,label,val,sub in [
        (k1,"Training countries","239","WHO COVID-19 global"),
        (k2,"Training rows","74,090","after feature engineering"),
        (k3,"Features","16","Rt + lags + rolling + z-score"),
        (k4,"Test diseases","2","COVID + RSV (cross-disease)"),
    ]:
    ...
    with r2:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "AUC":[0.923,0.881,0.549,0.518],"F1 (default)":[0.000,0.000,0.000,0.000],"F1 (Fix A)":[0.848,0.612,0.400,0.320],
        }),use_container_width=True,hide_index=True)
    ...
    f2c.markdown('<div class="box-blue"><strong>Cross-disease transfer</strong><br>Trained on COVID, tested on RSV. AUC=0.923. After threshold recalibration (Fix A), F1=0.848. The model learns epidemic shape, not disease-specific patterns.</div>',unsafe_allow_html=True)
...
# ── PAGE 2: WHO EARLY WARNING ──────────────────────────────────────────────────
elif "Surveillance" in page:
    st.markdown('''<div class="who-bar"><h1>\U0001f9a0 Inferra \u2014 Disease Surge Forecast</h1>
        <p>Powered by machine learning trained on 239 countries \u00b7 2-week and 4-week surge probability</p>
    </div>''', unsafe_allow_html=True)

    st.markdown('<div class="sec">COVID-19 \u2014 select a country</div>', unsafe_allow_html=True)
    who,pop_df = load_who()
    ...
    if len(feat_df)<10:
        ...
        # Small historical overview (full history, mini)
        ...
        if last_date.year < 2024:
            st.warning(f"⚠ **{selected}** stopped reporting COVID-19 data to WHO after {last_date.strftime('%b %Y')}. "
                       f"Surge forecast is based on the last available data and may not reflect current conditions.")
        ...
        fc1.markdown(f'''<div class="kpi">
            <div class="kpi-label">Current trend</div>
            <div class="kpi-value" style="color:{col_rt}">{ico_rt} {lbl_rt}</div>
            <div class="kpi-sub">R\u209c = {rt_c:.3f}</div>
        </div>''', unsafe_allow_html=True)

        fc2.markdown(f'''<div class="kpi">
            <div class="kpi-label">Surge risk \u2014 2 weeks</div>
            <div class="kpi-value" style="color:{col2}">{ico2} {lbl2}</div>
            <div class="kpi-sub">{p2*100:.0f}% model confidence</div>
        </div>''', unsafe_allow_html=True)

        fc3.markdown(f'''<div class="kpi">
            <div class="kpi-label">Surge risk \u2014 4 weeks</div>
            <div class="kpi-value" style="color:{col4}">{ico4} {lbl4}</div>
            <div class="kpi-sub">{p4*100:.0f}% model confidence</div>
        </div>''', unsafe_allow_html=True)
        ...
        interp_msg = (
            ...
            f"within the next 2\u20134 weeks. "
            f"Public health agencies should review surge preparedness protocols."
        ) if p2 > 0.6 or p4 > 0.6 else (
            f"There is a <strong>moderate surge signal for {selected}</strong>. "
            f"The model's R\u209c estimate of {rt_c:.3f} suggests monitoring is warranted "
            f"but no immediate action is required."
        ) if p2 > 0.3 or p4 > 0.3 else (
            f"The model shows <strong>low surge risk for {selected}</strong> over the next 4 weeks. "
            f"Current R\u209c = {rt_c:.3f} indicates transmission is {lbl_rt.lower()}."
        )
    ...
    st.markdown('<div class="sec">RSV \u2014 Australia (national surveillance)</div>', unsafe_allow_html=True)
    st.caption("RSV data is Australia-specific from NNDSS fortnightly reports. No country selection needed.")
...
# ── PAGE 3: PUBLIC SITUATION ───────────────────────────────────────────────────
    ...
    # ════════════════════════════════════════════════════════════
    # SECTION A — RSV Australia
    # ════════════════════════════════════════════════════════════
    st.markdown('<div class="sec">\U0001f7e6 RSV \u2014 Australia</div>', unsafe_allow_html=True)
    ...
    # RSV advice immediately below RSV chart
    st.markdown("*Source: WHO RSV fact sheet & Australian Department of Health*")
...
- Seek early care for infants with breathing difficulty""")
    ...
    # ════════════════════════════════════════════════════════════
    # SECTION B — COVID-19 (Australia + country dropdown)
    # ════════════════════════════════════════════════════════════
    st.markdown('<div class="sec">\U0001f534 COVID-19 \u2014 Australia & worldwide</div>', unsafe_allow_html=True)

    # COVID Australia status
    covid_aus = covid.copy()
    if "Rt" in covid_aus.columns and covid_aus["Rt"].notna().any():
        covid_rt_now = float(covid_aus.dropna(subset=["Rt"]).iloc[-1]["Rt"])
    else:
        covid_rt_now = 1.0
    ...
    if covid_rt_now>1.05:
        cv_color,cv_icon,cv_status,cv_msg,cv_box=("#c62828","\u26a0\ufe0f","SURGE RISK",
            "COVID-19 cases are growing in Australia. Each infected person is generating more than one new case.",
            "box-red")
    elif covid_rt_now>0.98:
        cv_color,cv_icon,cv_status,cv_msg,cv_box=("#f57f17","\u26a1","WATCH",
            "COVID-19 transmission is stable in Australia but could shift. Stay up to date with boosters.",
            "box-amber")
    else:
        cv_color,cv_icon,cv_status,cv_msg,cv_box=("#2e7d32","\u2713","DECLINING",
            "COVID-19 cases are declining in Australia. Continue routine precautions.",
            "box-green")
    ...
    # Single COVID chart with country dropdown
    ...
    # COVID advice immediately below COVID chart
...
- Emergency signs — breathing difficulty, chest pain: seek care immediately""")
    ...
    st.markdown("---")
    st.caption("Data: NNDSS Fortnightly Reports \u00b7 WHO COVID-19 Global Data \u00b7 ABS Population Estimates \u00b7 Inferra ML model (IST 718)")